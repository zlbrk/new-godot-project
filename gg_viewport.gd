...
# Label placement
# -----------------------------------------------------------------------------
const LOOP_LABEL_OFFSET: Vector2 = Vector2(0.0, -16.0)
const SURFACE_LABEL_OFFSET: Vector2 = Vector2(0.0, 16.0)
...
func _draw_centered_label(
	...
	var label_center: Vector2 = anchor + offset * GGTheme.ui_scale()

	var text_position: Vector2 = label_center + Vector2(
		-text_size.x * 0.5,
		text_size.y * 0.35
	)
...
func draw_axes() -> void:
	...
	var origin: Vector2 = world_to_screen(
		GGPoint2D.new(0, 0.0, 0.0)
	)
	...
	var x_text_size: Vector2 = font.get_string_size(
		"X",
		HORIZONTAL_ALIGNMENT_LEFT,
		-1,
		font_size
	...
	var y_text_size: Vector2 = font.get_string_size(
		"Y",
		HORIZONTAL_ALIGNMENT_LEFT,
		-1,
		font_size
...
func draw_surfaces() -> void:
	...
	for surface: GGSurface2D in model.surfaces:
		...
		var outer_loop: GGLoop2D = model.get_loop_by_id(
			surface.loop_ids[0]
		)
		...
		var outer_pts: Array[GGPoint2D] = model.get_loop_points(
			outer_loop
		)
		...
		var current_polys: Array[PackedVector2Array] = [
			outer_poly
		]

		for i: int in range(1, surface.loop_ids.size()):
			var hole_loop: GGLoop2D = model.get_loop_by_id(
				surface.loop_ids[i]
			)
			...
			var hole_pts: Array[GGPoint2D] = model.get_loop_points(
				hole_loop
			)
		...
		var centroid_world: Vector2 = model.get_loop_centroid(
			outer_loop
		)

		var centroid_screen: Vector2 = world_to_screen(
			GGPoint2D.new(
				0,
				centroid_world.x,
				centroid_world.y
...
func draw_loops() -> void:
	...
	var symbol_width_px: float = font.get_string_size(
		"L00 ",
		HORIZONTAL_ALIGNMENT_LEFT,
		-1,
		font_size
	...
	var symbol_width: float = symbol_width_px / GGTheme.ui_scale()

	for i: int in range(model.loops.size()):
		var loop: GGLoop2D = model.loops[i]
		...
		var loop_number: int = i + 1
		var text: String = "L%02d" % loop.id

		var dynamic_offset: Vector2 = LOOP_LABEL_OFFSET + Vector2(
			symbol_width * float(loop_number),
			0.0
		)
...
func draw_lines() -> void:
	...
	for line: GGLine2D in model.lines:
		var p_start: GGPoint2D = model.get_point_by_id(
			line.start_point_id
		)

		var p_end: GGPoint2D = model.get_point_by_id(
			line.end_point_id
		)
...
func draw_line_labels() -> void:
	...
	for line: GGLine2D in model.lines:
		...
		var text_pos: Vector2 = label_center - Vector2(
			text_size.x * 0.5,
			-text_size.y * 0.3
		)
